...
EXACT_FILES_TO_REMOVE = ['COVID.metadata.xlsx', 'README.md.txt', 
                         'Lung_Opacity.metadata.xlsx', 'Normal.metadata.xlsx', 
...
if os.path.isdir(destination_path) and len(os.listdir(destination_path)) > 0:
    print(f"Local dataset folder already exists and is not empty at: {destination_path}")
    sys.exit()
...
os.rename(os.path.join(destination_path, 'Viral Pneumonia'), os.path.join(destination_path, 'Non-COVID'))
os.rename(os.path.join(destination_path, 'Normal'), os.path.join(destination_path, 'Healthy'))  
