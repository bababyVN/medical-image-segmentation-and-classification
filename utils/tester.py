...
def test_all_models(device='cuda', batch_size=16):
    ...
    try:
        test_cls_dataset = ClassificationDataset(DATA_ROOT, test_cls_transform, split='test')
    ...
    try:
        test_seg_dataset = SegmentationDataset(DATA_ROOT, test_seg_transform, split='test')
        
        if len(test_seg_dataset) == 0:
            print(f"\n[WARNING] Segmentation test dataset is empty. Skipping segmentation testing.")
        else:
            test_seg_loader = DataLoader(test_seg_dataset, batch_size=batch_size // 2, 
                                        shuffle=False, num_workers=4, pin_memory=True)
...
def print_summary(results):
    ...
    if cls_models:
        ...
        for model in cls_models:
            ...
            print(f"{model:<20} {metrics['accuracy']:>10.2f}% {metrics['precision']:>10.2f}% "
    ...
    if seg_models:
        print("\n\nSEGMENTATION MODELS:")
        print("-"*80)
        print(f"{'Model':<20} {'IoU':<10} {'Dice':<10} {'Precision':<12} {'Recall':<12} {'F1 Score':<12}")
        print("-"*80)
        for model in seg_models:
            metrics = results[model]
            print(f"{model:<20} {metrics['iou']:>8.2f}% {metrics['dice']:>8.2f}% "
