    ...
    try:
        if "clip" in name_lower:
            ...
            model = torch.hub.load(
                "pytorch/vision:v0.10.0", name_lower, weights="IMAGENET1K_V1"
            )
        elif "vgg" in name_lower:
            hub_name = name_lower + "_bn"
            model = torch.hub.load(
                "pytorch/vision:v0.10.0", hub_name, weights="IMAGENET1K_V1"
            )
        else:
            raise ValueError(f"Unknown classification model: {name}")
    except Exception as e:
        raise RuntimeError(
            f"Failed to load base model architecture for {name}. Ensure PyTorch/torchvision are installed and compatible. Hub Error: {e}"
        )
...
def get_segmentation_model(name):
    ...
    model_configs = {
        "resnetunet": {"file": "ResnetUnet.py", "class": "ResNetUnet"},
        "attentionunet": {"file": "AttentionUNet.py", "class": "AttentionUNet"},
        "r2unet": {"file": "R2U_Net.py", "class": "R2U_Net"},
        "r2attunet": {"file": "R2AttU_Net.py", "class": "R2AttU_Net"},
    }
    if name_lower not in model_configs:
        raise ValueError(f"Unknown segmentation model: {name}")
    config = model_configs[name_lower]
    model_filepath = os.path.join(
        project_root, "models", "segmentation_models", config["file"]
    )
    try:
        ModelClass = _load_class_from_file(
            file_path=model_filepath, class_name=config["class"], module_name=name_lower
        )
        return ModelClass()
    except Exception as e:
        print(
            f"FATAL WARNING: Cannot import segmentation model architecture for {name}."
        )
        ...
        print(
            f"Please verify the file name is **{config['file']}** and the class inside is **{config['class']}** in the `models/segmentation_models/` folder."
        )
        print("***\n")
        return PlaceholderModel()
...
class Pipeline:
    ...
    def _load_models(self, classification_name, segmentation_name):
        ...
        if (
            not self.classification_model
            or self.classification_model.__class__.__name__.lower()
            ...
            try:
                ...
                self.classification_model = get_classification_model(
                    classification_name
                )
                cls_weights_path = os.path.join(
                    CLS_SAVE, f"{classification_name}_best_acc.pt"
                )
                ...
                if os.path.exists(cls_weights_path):
                    if "clip" in classification_name.lower():
                        ...
                        state_dict = torch.load(
                            cls_weights_path, map_location=DEVICE, weights_only=True
                        )
                        self.classification_model.clip_model.load_state_dict(
                        ...
                        if (
                            "classifier.7.weight" in state_dict
                            and "classifier.7.bias" in state_dict
                        ):
                            w = state_dict["classifier.7.weight"]
                            b = state_dict["classifier.7.bias"]
                            in_features = self.classification_model.classifier[
                                6
                            ].in_features
                            self.classification_model.classifier[6] = nn.Linear(
                                in_features, 3
                            )
                    ...
                    if "clip" not in classification_name.lower():
                        model_load_result = self.classification_model.load_state_dict(
                            state_dict, strict=False
                        )
                        if model_load_result.missing_keys:
                            print(
                                f"Missing keys (usually last layer): {model_load_result.missing_keys}"
                            )
                else:
                    print(
                        "Weights file not found. Using pretrained ImageNet weights only."
                    )

                print(
                    f"Successfully loaded Classification Model: {classification_name} (Features and intermediate classifier layers loaded; final layer is custom 3-class)."
                )
                success = True
                ...
                print(
                    f"CRITICAL ERROR: Failed to load classification model {classification_name}. File or general error: {e}"
                )
                self.classification_model = None
        ...
        if (
            ...
            or self.segmentation_model.__class__.__name__.lower()
            != segmentation_name.lower()
        ):
            self.segmentation_model = None
            try:
                ...
                if not isinstance(self.segmentation_model, PlaceholderModel):
                    seg_weights_path = os.path.join(
                        SEG_SAVE, f"{segmentation_name}_best_loss.pt"
                    )
                    ...
                    if "clipseg" in segmentation_name.lower():
                        ...
                        state_dict = torch.load(
                            seg_weights_path, map_location=DEVICE, weights_only=True
                        )
                        self.segmentation_model.clipseg_model.load_state_dict(
                            state_dict, strict=False
                        )
                    ...
                    print(
                        f"Successfully loaded Segmentation Model: {segmentation_name}"
                    )
                else:
                    print(
                        f"Using Placeholder for Segmentation Model: {segmentation_name}. Segmentation will be skipped."
                    )
            except Exception as e:
                print(
                    f"CRITICAL ERROR: Failed to load segmentation weights for {segmentation_name}. Check weights path or architecture mismatch. Error: {e}"
                )
                self.segmentation_model = None
    ...
    def _predict_segmentation(self, img_tensor, pil_image=None):
        if self.segmentation_model is None or isinstance(
            self.segmentation_model, PlaceholderModel
        ):
            return None
        with torch.no_grad():
    ...
    def process_image(self, pil_image, segmentation_model_name, overlay_opacity=0.5):
        ...
        cls_name_to_load = (
            self.classification_model.__class__.__name__
            ...
            if mask_np is not None:
                ...
                if original_bgr.size > 0:
                    ...
                    blended_bgr = cv2.addWeighted(
                        original_bgr, 1.0, overlay, overlay_opacity, 0
                    )
