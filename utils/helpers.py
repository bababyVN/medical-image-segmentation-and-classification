...
def visualize_samples(
    n_samples: int = 6, classes: list[str] = CLASSES, data_root: str = DATA_ROOT
):
    ...
    for row, cls in enumerate(classes):
        ...
        for col, img_file in enumerate(img_files):
            ...
            axes[row, col].set_title(
                f"{cls}\n{os.path.basename(img_file)}", fontsize=10
            )
...
def visualize_image_mask_pairs(
    n_samples: int = 3, classes: list[str] = CLASSES, data_root: str = DATA_ROOT
):
...
def add_dropout_to_fc(model, p=0.5, classes=CLASSES):
    ...
    if hasattr(model, "fc"):
        ...
        model.fc = nn.Sequential(nn.Dropout(p=p), nn.Linear(num_features, len(classes)))
        ...
        new_classifier.extend(
            [nn.Dropout(p=p), nn.Linear(last_in_features, len(classes))]
        )
...
def get_class_model(name):
    ...
    try:
        ...
        print(
            f"Error loading model {name} from PyTorch Hub: {e}. Model will train from scratch."
        )
        ...
        if "resnet" in name_lower:
            ...
            ModelClass = (
                ResNet.ResNet18 if name_lower == "resnet18" else ResNet.ResNet50
            )
        elif "vgg" in name_lower:
...
# [Metrics] (Unchanged)
def acc(logits, y):
    return (torch.argmax(logits, 1) == y).sum().item(), y.size(0)


def iou(pred, mask, t=0.5):
    p = (pred > t).float()
    inter = (p * mask).sum()
    union = ((p + mask) > 0).float().sum()
...
def train(
    ...
    criterion = (
        nn.BCEWithLogitsLoss() if seg else nn.CrossEntropyLoss(label_smoothing=0.1)
    )
    ...
    # CLASSIFICATION: Two-Stage Fine-Tuning
        ...
        print(
            f"--- STAGE 1: Feature Extraction (Epochs {start_epoch}-{FEATURE_EXTRACTION_EPOCHS}) ---"
        )
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=FEATURE_EXTRACTION_EPOCHS
        )
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if not seg and epoch == FEATURE_EXTRACTION_EPOCHS + 1:
            ...
            print(
                f"Full fine-tuning (all layers unfrozen) with very low LR: {lr}. Using ReduceLROnPlateau scheduler."
            )
        ...
        with torch.no_grad():
            for x, y in val_dl:
                ...
                if seg:
                    val_metric += iou(torch.sigmoid(out), y)
                else:
                    preds = torch.argmax(out, 1)
        ...
        if seg:
            ...
            print(
                f"[{name}] Ep{epoch}: TrainLoss {running_loss/len(train_dl.dataset):.3f} | ValLoss {val_loss:.3f} | IoU {val_iou:.3f}"
            )
            ...
            print(
                f"[{name}] Ep{epoch}: TrainLoss {running_loss/len(train_dl.dataset):.3f} (Acc {train_acc:.2f}%) | ValLoss {val_loss:.3f} | ValAcc {val_acc:.2f}%"
            )
    ...
    end_time = time.time()
    print(
        f"Training for {name} finished in {(end_time - start_time) / 60:.2f} minutes."
    )
    return best_score
